...
void ConstantQ::compute(
  ...
  if (frame.size() != _windowSize) {
    ostringstream msg;
    msg << "ConstantQ: input frame size must be equal to: " << _windowSize;
    throw std::runtime_error(msg.str());
  }

  _fft->compute(frame, _fftData);
...
void ConstantQ::configure(const Config& config) {
  _sampleRate = config.sampleRate;
  _minFrequency = config.minFrequency;
  _numberBins = config.numberBins;
  _binsPerOctave = config.binsPerOctave;
  _threshold = config.threshold;
  _scale = config.scale;
  _minimumKernelSize = config.minimumKernelSize;
  _zeroPhase = config.zeroPhase;

  Windowing::Config windowingConfig;
  windowingConfig.type = config.windowType;
  windowingConfig.zeroPhase = false;
  _windowing->configure(windowingConfig);
  ...
  FFTW::Config fftConfig;
  fftConfig.size = _windowSize;
  _fft->configure(fftConfig);
  ...
  for (int k = _numberBins - 1; k > -1; k--) {
    ...
    vector<Real> window;
    _windowing->compute(unity, window);
    ...
    _fftc->compute(binKernel, binKernelFFT);
