...
void Windowing::configure(const Config& config) {
  _normalized = config.normalized;
  _window.resize(config.size);
  createWindow(config.type);
  _type = config.type;
  _zeroPadding = config.zeroPadding;
...
void Windowing::compute(
  ...
  std::vector<Real>& windowedSignal) {

  if (signal.size() <= 1) {
    throw std::runtime_error("Windowing: frame size should be larger than 1");
  }
